...
import { getStorefrontAcademyCourseBySlug } from '@/server/storefront/academy-courses';
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const detail = await getStorefrontAcademyCourseBySlug(slug, locale);
  if (!detail) {
    return NextResponse.json({ code: 'NOT_FOUND', message: 'Course not found' }, { status: 404, headers: frontCorsHeaders(request.headers.get('origin')) });
  }
