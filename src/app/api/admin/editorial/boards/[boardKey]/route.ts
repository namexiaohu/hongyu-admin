...
import {
  deleteEditorialCoverageBoard,
  getEditorialCoverageBoardDetail,
  patchEditorialCoverageBoardSchema,
  setEditorialCoverageBoardEnabled,
...
function mapError(error: unknown) {
  ...
  switch (error.message) {
    ...
    case 'BOARD_HAS_CONTENT':
      return { status: 409, code: 'BOARD_HAS_CONTENT', message: '该看板下已有内容，无法删除' };
    default:
      return null;
  }
}
...
export async function GET(_: Request, { params }: { params: Promise<{ boardKey: string }> }) {
  ...
  const item = await getEditorialCoverageBoardDetail(boardKey);
  if (!item) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ boardKey: string }> }) {
  ...
  const parsed = patchEditorialCoverageBoardSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ code: 'VALIDATION_ERROR', message: 'Invalid payload', details: parsed.error.flatten() }, { status: 400 });
  }

  const item = await setEditorialCoverageBoardEnabled(boardKey, parsed.data.enabled);
  if (!item) {
    return NextResponse.json({ code: 'NOT_FOUND', message: '看板不存在' }, { status: 404 });
  }
...
export async function DELETE(_: Request, { params }: { params: Promise<{ boardKey: string }> }) {
  ...
  try {
    const deleted = await deleteEditorialCoverageBoard(boardKey);
    if (!deleted) {
      return NextResponse.json({ code: 'NOT_FOUND', message: '看板不存在' }, { status: 404 });
    }
    const dashboard = await getAdminEditorialDashboard();
