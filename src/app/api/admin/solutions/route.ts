...
import { adminSolutionCreateSchema, type SolutionStatus } from '@/lib/solution-content';
import { createAdminSolution, getAdminSolutionList } from '@/server/admin/solutions';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = adminSolutionCreateSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ code: 'VALIDATION_ERROR', message: 'Invalid payload', details: parsed.error.flatten() }, { status: 400 });
  }

  try {
    const created = await createAdminSolution(parsed.data);
