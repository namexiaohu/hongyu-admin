...
const SHARED_IMAGE_SOURCES: SolutionSeedRecord['imageSources'] = {
...
function compactSolutionBlocks(overviewTitle: string, overviewDesc: string): SolutionSeedBlock[] {
  return [
    {
      type: 'split',
      layout: 'image-right',
    ...
    },
    {
      ...
      largeTitle: 'Core capabilities',
      description: 'Designed for repeatable outcomes in busy veterinary hospitals.',
      items: [
        {
          icon: 'layers',
          imageRole: 'feature1',
          largeTitle: 'Precision engineering',
          description: 'Tolerances and materials selected for companion-animal anatomy.',
        },
        {
          icon: 'shield',
          imageRole: 'feature2',
          largeTitle: 'Clinical validation',
          description: 'Bench and cadaver testing before field deployment.',
        },
        {
          icon: 'activity',
          imageRole: 'feature3',
          largeTitle: 'Workflow integration',
          description: 'Packaged to fit standard sterilization and OR turnover.',
        },
      ],
    },
    {
...
function extraSolution(input: {
  ...
  overviewDesc: string;
  imageSources?: SolutionSeedRecord['imageSources'];
}): SolutionSeedRecord {
  return {
    slug: input.slug,
    ...
    title: input.title,
    largeTitle: input.title,
    description: input.description,
    seoTitle: `${input.title} | Hongyu Medical`,
    seoDescription: input.description,
    coverRole: 'cover',
    stats: input.stats,
    productParams: input.productParams,
    tags: input.tags,
    imageSources: input.imageSources ?? SHARED_IMAGE_SOURCES,
    blocks: compactSolutionBlocks(input.overviewTitle, input.overviewDesc),
  };
}
...
export const SOLUTION_SEED_RECORDS: SolutionSeedRecord[] = [
  ...
  extraSolution({
    ...
    categorySlug: 'v-clamp',
    sortOrder: 4,
    badgeText: 'Compact Model',
    title: 'V-CLAMP Mini Closure System',
    description:
      'A downsized vascular closure platform for cats and toy-breed dogs, with the same adaptive clamping logic as the flagship V-CLAMP in a lighter footprint.',
    tags: ['Compact Footprint', 'Adaptive Clamping', 'Feline Optimized'],
    stats: [
      { label: 'Applicable vessel diameter', value: '1–4 mm' },
      { label: 'Generator weight reduction', value: '35%' },
      { label: 'Closure response time', value: '≤ 1.2 s' },
      { label: 'Pilot hospital sites', value: '24' },
    ],
    productParams: [
      { label: 'Product models', value: 'V-CLAMP Mini 50 / 100' },
      { label: 'Species', value: 'Cats and dogs under 8 kg' },
      { label: 'Clamping-force range', value: '3 N – 10 N' },
      { label: 'Sterilization', value: 'EO / gamma' },
    ],
    overviewTitle: 'Closure performance for smaller patients',
    overviewDesc:
      'V-CLAMP Mini brings adaptive hemostasis to laparoscopic and open procedures where standard clamps are oversized, without sacrificing capture thresholds in thin-walled vessels.',
  }),
  extraSolution({
    slug: 'laparoscopic-trocar',
    categorySlug: 'v-clamp',
    sortOrder: 5,
    badgeText: 'Access Platform',
    title: 'Laparoscopic Trocar System',
    description:
      'Bladeless, radiolucent trocars with adjustable sleeve length for canine and feline minimally invasive abdominal surgery.',
    tags: ['Bladeless Tip', 'Radiolucent Sleeve', 'Adjustable Length'],
    stats: [
      { label: 'Cannula diameters', value: '5–12 mm' },
      { label: 'Seal cycles tested', value: '500+' },
      { label: 'Insertion force reduction', value: '18%' },
      { label: 'Hospitals in evaluation', value: '32' },
    ],
    productParams: [
      { label: 'Sizes', value: '5 mm / 8 mm / 12 mm' },
      { label: 'Sleeve adjustment', value: '40–120 mm' },
      { label: 'Valve type', value: 'Silicone duckbill' },
      { label: 'Certifications', value: 'ISO 13485, CE' },
    ],
    overviewTitle: 'Reliable access for MIS abdominal cases',
    overviewDesc:
      'The trocar line pairs with Hongyu laparoscopic stacks to standardize port placement, reduce gas leak, and support repeat sterilization in high-volume spay/neuter and soft-tissue programs.',
    imageSources: {
      ...SHARED_IMAGE_SOURCES,
      cover: { remoteUrl: VET_CLINIC },
    },
  }),
  extraSolution({
    slug: 'stifle-anchor-system',
    categorySlug: 'sports-medicine',
    sortOrder: 6,
    badgeText: 'Stifle Focus',
    title: 'Stifle Anchor System',
    description:
      'A dedicated anchor and drill-guide set for extracapsular and intracapsular stifle reconstruction in medium and large dogs.',
    tags: ['Drill Guides', 'PEEK Anchors', 'Stifle Workflow'],
    stats: [
      { label: 'Anchor pull-out strength', value: '450 N+' },
      { label: 'Guide accuracy', value: '±1 mm' },
      { label: 'Procedure time saved', value: '12 min' },
      { label: 'Surgeon training sites', value: '16' },
    ],
    productParams: [
      { label: 'Indication', value: 'CCL rupture, stifle instability' },
      { label: 'Anchor sizes', value: '4.5 mm / 5.5 mm' },
      { label: 'Guide angles', value: '45° / 55° / 65°' },
      { label: 'Material', value: 'PEEK / titanium alloy' },
    ],
    overviewTitle: 'Standardized stifle reconstruction',
    overviewDesc:
      'Bone tunnels and anchor seating are aligned to canine tibial anatomy, helping teams move from diagnosis to stable fixation with fewer instrument swaps.',
  }),
  extraSolution({
    slug: 'meniscal-repair-kit',
    categorySlug: 'sports-medicine',
    sortOrder: 7,
    badgeText: 'Meniscus Line',
    title: 'Meniscal Repair Kit',
    description:
      'All-inside and outside-in meniscal repair instruments with UHMWPE suture options sized for canine stifle joints.',
    tags: ['All-inside Repair', 'UHMWPE Suture', 'Arthroscopic Ready'],
    stats: [
      { label: 'Suture strengths', value: '80–120 N' },
      { label: 'Scope compatibility', value: '2.7 mm+' },
      { label: 'Implant retrieval rate', value: '<2%' },
      { label: 'Cases documented', value: '1,200+' },
    ],
    productParams: [
      { label: 'Techniques supported', value: 'All-inside, outside-in' },
      { label: 'Needle curvature', value: '15° / 25°' },
      { label: 'Suture options', value: 'UHMWPE, absorbable PGA' },
      { label: 'Sterilization', value: 'EO' },
    ],
    overviewTitle: 'Preserve meniscal tissue when possible',
    overviewDesc:
      'The kit supports partial meniscectomy alternatives in CCL cases, with instruments that fit standard arthroscopic towers used in referral hospitals.',
    imageSources: {
      ...SHARED_IMAGE_SOURCES,
      cover: { remoteUrl: MED_DEVICE },
    },
  }),
  extraSolution({
    slug: 'wireless-holter',
    categorySlug: 'pacemaker',
    sortOrder: 8,
    badgeText: 'Diagnostics',
    title: 'Wireless Holter Monitor',
    description:
      'A lightweight, patch-style Holter for 24–72 hour ambulatory ECG in dogs and cats, used to stratify bradyarrhythmia candidates before pacemaker implant.',
    tags: ['Patch Electrode', '72-hour Capture', 'Cloud Review'],
    stats: [
      { label: 'Continuous recording', value: '72 hr' },
      { label: 'Device mass', value: '18 g' },
      { label: 'Arrhythmia detection sensitivity', value: '96%' },
      { label: 'Cardiology clinics', value: '12' },
    ],
    productParams: [
      { label: 'Recording duration', value: '24 / 48 / 72 hours' },
      { label: 'Species', value: 'Dogs and cats, 2–25 kg' },
      { label: 'Data export', value: 'PDF / HL7-compatible XML' },
      { label: 'Regulatory status', value: 'Clinical evaluation' },
    ],
    overviewTitle: 'Pre-implant rhythm characterization',
    overviewDesc:
      'Holter data feeds the pacemaker trial workflow, helping cardiologists confirm pause burden and select generator programming before generator placement.',
    imageSources: {
      ...SHARED_IMAGE_SOURCES,
      cover: { remoteUrl: HEART_MODEL },
    },
  }),
  extraSolution({
    slug: 'optical-navigation-beta',
    categorySlug: 'in-research',
    sortOrder: 9,
    badgeText: 'Beta Program',
    title: 'Optical Navigation Beta',
    description:
      'Marker-based optical tracking for corrective osteotomy and implant alignment, currently in concept validation with four partner hospitals.',
    tags: ['Sub-millimeter Tracking', 'Osteotomy Guidance', 'Partner Beta'],
    stats: [
      { label: 'Registration error target', value: '<1 mm' },
      { label: 'Beta sites', value: '4' },
      { label: 'Calibration time', value: '<5 min' },
      { label: 'Supported procedures', value: '6' },
    ],
    productParams: [
      { label: 'Tracking modality', value: 'Passive optical markers' },
      { label: 'Camera baseline', value: '1.2 m recommended' },
      { label: 'Software', value: 'Research build v0.9' },
      { label: 'Availability', value: 'Invitation-only beta' },
    ],
    overviewTitle: 'Navigation without human-scale hardware',
    overviewDesc:
      'The beta program adapts optical navigation to veterinary OR layouts, focusing on tibial plateau leveling and long-bone osteotomy workflows.',
    imageSources: {
      ...SHARED_IMAGE_SOURCES,
      cover: { remoteUrl: LAB_BENCH },
    },
  }),
];
