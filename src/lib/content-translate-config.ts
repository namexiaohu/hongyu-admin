...
export const CONTENT_TRANSLATE_PROFILES: Record<ContentTranslateType, ContentTranslateProfile> = {
  ...
  product: {
    ...
    passthroughFields: ['coverUrl', 'videoUrl', 'galleryJson', 'attachmentsJson'],
    serverLabel: 'catalog product',
    tooltip: '将默认语言已保存的产品名称、角标、附加文案、描述、数据指标、认证与 SEO 字段翻译到当前语言，并按汇率换算销售价与原价；封面图、视频、图集与附件直接沿用默认语言；Slug 不会翻译；详细描述保留排版结构',
  },
  feature: {
