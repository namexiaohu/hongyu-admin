...
export function peekSharedCoverMediaAssets(): AdminMediaAsset[] | null {
  return cachedItems;
}

export async function getSharedCoverMediaAssets(options?: {
  force?: boolean;
  ...
  const request = (async () => {
    const response = await fetch(`/api/admin/media-assets?type=${MEDIA_ASSET_TYPE_COVER}`);
    if (!response.ok) throw new Error('加载封面图库失败');
...
export function setSharedCoverMediaAssets(items: AdminMediaAsset[]) {
  cachedItems = items;
}

export function removeSharedCoverMediaAsset(id: string) {
