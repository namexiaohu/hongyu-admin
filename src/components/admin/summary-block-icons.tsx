...
const summaryIconSvgs: Record<ContentBlockSummaryIcon, ReactNode> = {
  layers: (
    <svg {...iconStroke}>
...
type SummaryBlockIconPickerProps = {
  value?: string | null;
  onChange: (icon: ContentBlockSummaryIcon | null) => void;
};
...
export function SummaryBlockIconSvg({ icon }: { icon: ContentBlockSummaryIcon }) {
  return summaryIconSvgs[icon];
}

export function SummaryBlockIconPicker({ value, onChange }: SummaryBlockIconPickerProps) {
  const selectedValue = value && isContentBlockSummaryIcon(value) ? value : null;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {contentBlockSummaryIcons.map((icon) => {
