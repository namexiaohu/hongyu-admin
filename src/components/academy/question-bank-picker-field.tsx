...
import { formatAcademyQuestionBankSelectedDisplay } from '@/lib/academy-question-bank-content';

import { QuestionBankPickerModal } from './question-bank-picker-modal';

type Props = {
  value?: string[];
  onChange?: (value: string[]) => void;
  seedItems?: AcademyQuestionBankPickerItem[];
  loading?: boolean;
  disabled?: boolean;
};

async function lookupBanks(ids: string[]): Promise<AcademyQuestionBankPickerItem[]> {
  if (!ids.length) return [];
  const response = await fetch('/api/admin/academy/question-banks/picker', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ids }),
  });
  if (!response.ok) return [];
  const payload = (await response.json()) as { items: AcademyQuestionBankPickerItem[] };
  return payload.items ?? [];
}

function bankStub(id: string): AcademyQuestionBankPickerItem {
  return { id, title: id, questionCount: 0, totalScore: 0, passScorePercent: 60 };
}

export function QuestionBankPickerField({ value = [], onChange, seedItems, loading = false, disabled = false }: Props) {
  const [pickerOpen, setPickerOpen] = useState(false);
  const [cache, setCache] = useState<Map<string, AcademyQuestionBankPickerItem>>(new Map());
      ...
      .then((items) => {
        ...
        setCache((current) => {
          ...
          for (const id of missingIds) {
            if (!next.has(id)) next.set(id, bankStub(id));
          }
          return next;
        });
      });
  ...
  const selectedItems = useMemo(
    () => selectedIds.map((id) => {
      const item = cache.get(id) ?? bankStub(id);
      const display = formatAcademyQuestionBankSelectedDisplay(item);
      return { id, name: display.name, meta: display.meta };
    }),
    [cache, selectedIds],
  );
      ...
      <div className="entity-picker-field" style={showSpinner ? { minHeight: 120 } : undefined}>
        <div className="entity-picker-selected">
          {!ready ? null : !selectedItems.length ? (
            <Typography.Text type="secondary">尚未选择题库</Typography.Text>
          ) : (
