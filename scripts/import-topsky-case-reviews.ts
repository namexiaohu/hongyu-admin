...
type GetDataResponse = {
  attributes?: {
    config?: Record<string, { dataSource?: { settingValues?: { sourceConfig?: { items?: ListItem[]; pager?: { totalPages?: number; totalRecords?: number; currentPage?: number } } } } }>;
  };
  pageInfo?: {
    name?: string;
    path?: string;
...
function buildSummary(bodyHtml: string, fallback?: string) {
  ...
  if (!text) {
    ...
    if (!intro) return 'Case review from HongYu Medical Case Sharing.';
...
function injectHeadingSections(html: string) {
  ...
  if (blocks.length <= 1) {
    ...
    if (!text) return html;
    return `<h2>Case Overview</h2>\n${html}`;
  }

  const chunkSize = Math.max(1, Math.ceil(blocks.length / Math.min(SECTION_TITLES.length, Math.max(2, Math.ceil(blocks.length / 3)))));
...
function extractListItems(data: GetDataResponse): { items: ListItem[]; totalPages: number } {
  const config = data.attributes?.config ?? {};
  const tibKey = Object.keys(config).find((key) => key.startsWith('TIBTextList'));
  if (!tibKey) {
    throw new Error('TIBTextList config not found on case-sharing page');
  }
  const source = config[tibKey]?.dataSource?.settingValues?.sourceConfig;
  const items = source?.items ?? [];
  const totalPages = source?.pager?.totalPages ?? 1;
  return { items, totalPages };
}

async function fetchAllListItems() {
  const first = await fetchGetData('/case-sharing', 1);
  const { items: page1, totalPages } = extractListItems(first);
  const all = [...page1];

  for (let page = 2; page <= totalPages; page += 1) {
    const data = await fetchGetData('/case-sharing', page);
...
async function ensureCaseBoardExists() {
  const [board] = await db!
    .select({ id: editorialCoverageBoards.id })
    .from(editorialCoverageBoards)
    .where(eq(editorialCoverageBoards.boardKey, BOARD_KEY))
    .limit(1);
  if (!board) {
    throw new Error('看板 case 不存在，请先在后台创建「病例回顾」看板');
  }
}
...
async function syncBoard(contentId: string) {
...
async function listExistingCaseContentIds() {
  const rows = await db!
    .select({ contentId: editorialContentBoards.contentId })
    .from(editorialContentBoards)
    .where(eq(editorialContentBoards.boardKey, BOARD_KEY));
...
async function upsertArticle(input: {
  ...
  if (existing) {
    ...
    await syncBoard(existing.contentId);
...
async function purgeOldCaseArticles(keepContentIds: string[], preexistingIds: string[]) {
  const keepSet = new Set(keepContentIds);
  const toDelete = preexistingIds.filter((id) => !keepSet.has(id));
  if (!toDelete.length) {
    console.log('无需删除旧病例回顾文章');
    return 0;
  }

  // Confirm they are still on the case board before deleting content rows.
  const deletable = await db!
    .select({ contentId: editorialContentBoards.contentId })
    .from(editorialContentBoards)
    .where(
      and(
        eq(editorialContentBoards.boardKey, BOARD_KEY),
  ...
  console.log(`已删除旧病例回顾文章 ${ids.length} 篇`);
  return ids.length;
}
...
async function processArticle(item: ListItem) {
  ...
  if (!APPLY) {
    ...
    return { slug, contentId: null as string | null, created: false };
  }
  ...
  console.log(`  ${result.created ? '新建' : '更新'} contentId=${result.contentId}`);
  return { slug, contentId: result.contentId, created: result.created };
}
...
async function main() {
  ...
  await ensureCaseBoardExists();

  const preexistingIds = await listExistingCaseContentIds();
  console.log(`当前病例回顾文章数: ${preexistingIds.length}`);

  console.log('拉取 Case Sharing 列表...');
  const items = await fetchAllListItems();
  console.log(`列表共 ${items.length} 篇`);

  let created = 0;
  let updated = 0;
  const importedContentIds: string[] = [];

  for (const item of items) {
    const result = await processArticle(item);
    if (result.contentId) {
      importedContentIds.push(result.contentId);
  ...
  if (APPLY) {
    const deleted = await purgeOldCaseArticles(importedContentIds, preexistingIds);
    console.log('\n完成:', { imported: items.length, created, updated, deleted, kept: importedContentIds.length });
  } else {
    const wouldDelete = preexistingIds.length; // 导入后旧文会清掉；dry-run 无法预知 upsert 命中，提示现有数量
    console.log('\nDRY-RUN 完成:', {
      wouldImport: items.length,
      existingCaseArticles: preexistingIds.length,
      note: '执行 pnpm db:import:topsky-cases -- --apply 才会上传 R2、写库并清理旧 case 文章',
      wouldDeleteHint: wouldDelete,
    });
  }
}
